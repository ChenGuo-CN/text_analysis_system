...
// 使用stb_image读取JPEG图像（替代turbojpeg）
static int read_image_jpeg(const char* path, image_buffer_t* image)
{
    ...
    if (!data) {
        printf("error: failed to load jpeg image %s\n", path);
        return -1;
    }
    
    printf("origin size=%dx%d\n", width, height);
    
    // 对图像做裁剪16对齐，利于后续rga操作
    int crop_width = width / 16 * 16;
    int crop_height = height / 16 * 16;
    
    printf("input image: %d x %d, channels: %d\n", width, height, channels);
    ...
    memcpy(sw_out_buf, data, sw_out_size);
    stbi_image_free(data);
...
// 使用stb_image_write写入JPEG图像（替代turbojpeg）
static int write_image_jpeg(const char* path, int quality, const image_buffer_t* image)
{
    ...
    if (image->format != IMAGE_FORMAT_RGB888) {
        printf("write_image_jpeg: only RGB888 format is supported\n");
        return -1;
    }
    
    int ret = stbi_write_jpg(path, width, height, 3, data, quality);
    if (ret == 0) {
        printf("error: failed to write jpeg image %s\n", path);
        return -1;
    }
